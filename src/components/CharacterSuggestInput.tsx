  ...
  return (
    <div className="relative flex flex-col gap-1  w-1/3">
      <div
        ...
        className="px-2 shadow-sm rounded-md relative w-full flex items-center border-[0.1px] border-[#a5a5a58a] "
      >
        <input
      ...
      <ul
        className={`${open && selectedCharacter.trim().length > 0 ? 'absolute max-h-50 ' : 'hidden'} overflow-y-auto small-scroll custom-scroll top-10 px-2 bg-white border-[0.1px] border-[#a5a5a58a] rounded-md shadow-md inset-x-0`}
      >
        {selectedCharacter.trim().length > 0 &&
          suggestions.map((suggestion) => (
            <li
              onClick={() => handleSelect(suggestion)}
              className="py-[6px] text-sm my-1 px-2 hover:bg-[#F0F0F0] cursor-pointer active:bg-[#f0f0f09c] hover:rounded-md"
              key={suggestion}
            >
              {suggestion}
            </li>
