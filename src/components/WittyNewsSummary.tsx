...
const countryNames = (Object.values(countries) as { name: string }[]).map(
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex flex-col gap-1 w-full md:w-1/2">
            ...
            <input
              ...
              className="placeholder:text-[#80808075] border-[0.1px] border-[#a5a5a58a] rounded-md px-2 py-[6px] outline-none shadow-sm text-sm placeholder:text-sm"
            />
            <div className="flex justify-between items-center">
              ...
              >
                <span
                  className={`block ml-[1px] w-5 h-5 bg-white rounded-full shadow-md transform transition-transform duration-300 ${enabled ? 'translate-x-4' : 'translate-x-0'}`}
                ></span>
              </button>
            </div>
          </div>
          ...
          <div className="relative flex flex-col gap-1 w-full md:w-1/2">
            ...
            >
              <span
                className={`text-sm ${selectedTime ? 'text-gray-800' : 'text-[#80808075]'}`}
              >
            ...
            <p className="text-xs text-[#0000006c]">
              Your daily newsletter will be sent at the time you select here
            </p>
          </div>
        </div>
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex flex-col gap-1 w-full md:w-1/2">
            ...
            <div className="relative flex flex-col gap-1">
              ...
              {selected.length === 0 && (
                <p className="text-xs text-[#0000006c]">
                  Select regions to get news from
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1 w-full md:w-1/2">
            ...
            <div className="relative flex flex-col gap-1">
              ...
              {selectedCategories.length === 0 && (
                <p className="text-xs text-[#0000006c]">
                  Select the topics or industries you want news about
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-1">
            <div className="flex w-full justify-between items-center">
              ...
              <button
                onClick={() => {
                  setDailyInsightsEnabled(!dailyInsightsEnabled);
                }}
                className="h-6 w-10 bg-black rounded-full relative"
              >
                <span
                  className={`block ml-[1px] w-5 h-5 bg-white rounded-full shadow-md transform transition-transform duration-300 ${dailyInsightsEnabled ? 'translate-x-4' : 'translate-x-0'}`}
                ></span>
              </button>
            </div>
            <div className="flex w-full justify-between items-center">
              ...
              <p className="text-xs text-[#0000006c] flex-shrink-0 ml-4">
                {dailyInsightsEnabled
                  ? 'Daily Insights is on'
                  : 'Daily Insights is off'}
              </p>
            </div>
          </div>
          <button
            onClick={handleSave}
            className="w-fit hover:bg-[#111111dc] h-fit text-[#fffffff5] bg-black text-sm font-normal rounded-md px-2 py-[7px] tracking-wide"
          >
          ...
          {error && (
            <button
              onClick={handleSave}
              className="w-fit hover:bg-[#111111dc] h-fit text-[#df4646f5] bg-white border-[0.1px] border-black text-sm font-normal rounded-md px-2 py-[7px] tracking-wide"
            >
              {' '}
              Error occurred
            </button>
