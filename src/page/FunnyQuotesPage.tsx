...
const FunnyQuotesPage = ({ openSidebar, setOpenSidebar }: any) => {
    ...
    return (
        <div className="h-screen flex flex-col">
                ...
                {funnyQuotesResponse?.length > 0 ? (
                    <div className="max-w-xl mx-auto py-2 px-4 bg-white">
                        ...
                        {funnyQuotesResponse.map((data: any, index: any) => (
                            <div key={index} className="max-w-xl border-[0.1px] border-[#ecebeb] mx-auto my-8 p-4 bg-white rounded-lg shadow-sm hover:shadow-md">
            ...
            <button onClick={handleSubmit} className="curser-pointer  sticky bottom-10 mt-20 flex justify-center items-center w-full">
                <div tabIndex={0} className="w-fit h-fit focus:shadow-md flex items-center justify-center gap-3 py-3 border-[0.5px] border-[#cecece] focus:bg-[#f9f9f9] hover:bg-[#f9f9f9] rounded-full px-6 shadow z-50 bg-[#fff]">
                    <p className="font-normal">More Funny Quotes</p>
                    <img src="/Loading1.svg" alt="Loading" className={`${loading && "animate-spin"} font-light text-white`} />
                </div>
            </button>
        </div>
    )
