...
const FunnyCharactersPage = ({ openSidebar, setOpenSidebar }: any) => {
  ...
  const { funnyCharacterRequest, funnyCharacterResponse, loading } =
    useFunnyCharacter();
            ...
            {loading ? (
              <div className="flex flex-col justify-center items-center w-full h-full">
                <div className="flex items-center gap-2 w-fit px-auto">
                  ...
                  <p className="text-[#808080] font-semibold text-lg">
                    Loading...
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-8">
                <div className="flex flex-col gap-2 items-stretch">
                  {funnyCharacterResponse?.title && (
                    <h3 className="font-bold text-lg w-full">
                      {funnyCharacterResponse?.title}
                    </h3>
                  )}
                  {funnyCharacterResponse?.body && (
                    <p className="mt-2 w-full">
                      {funnyCharacterResponse?.body}
                    </p>
                  )}
                  {funnyCharacterResponse?.advice && (
                    <p className="italic mt-2 w-full">
                      {funnyCharacterResponse?.advice}
                    </p>
                  )}
                  {funnyCharacterResponse?.fun && (
                    <p className="mt-2 w-full">{funnyCharacterResponse?.fun}</p>
                  )}
                </div>
              </div>
            ) : (
              <div className="w-full h-full flex justify-center items-center">
                <p className="text-[#000000d0] text-center md:text-3xl">
                  Get your daily dose of savage roasts! 😎
                </p>
              </div>
            )}
          </div>
        ...
        <div className="sticky bottom-0 left-0 right-0 w-full rounded-t-xl flex flex-col bg-[#ffffff]">
          ...
          >
            <textarea
              ...
              placeholder={
                selectedCharacter.trim().length > 0
                  ? `Ask ${selectedCharacter} anything`
